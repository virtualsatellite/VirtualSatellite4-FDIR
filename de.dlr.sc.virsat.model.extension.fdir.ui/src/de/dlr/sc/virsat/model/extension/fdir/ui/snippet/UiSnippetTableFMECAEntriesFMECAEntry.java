...
public class UiSnippetTableFMECAEntriesFMECAEntry extends AUiSnippetTableFMECAEntriesFMECAEntry implements IUiSnippet {
	...
	/**
	 * Label provider that wraps a label provider and extends it to mark the critical field
	 * @author muel_s8
	 *
	 */
	private static class CriticalityLabelProvider extends VirSatTransactionalAdapterFactoryLabelProvider
		 ...
		 * @param adapterFactory the adapater factory
		 */
		...
		public Color getBackground(Object object, int columnIndex) {
			...
			if (columnIndex == CRITICALITY_COLUMN) {
				if (fmecaEntry.getCriticality() == 0) {
					...
					StructuralElementInstance root = (StructuralElementInstance) VirSatEcoreUtil.getRootContainer(fmecaEntry.getParent().getStructuralElementInstance());
					...
					if (!fdirParameters.isEmpty()) {
						final int DETECTABLE = 1;
						int severity = (int) fmecaEntry.getSeverityEnum();
						int probability = (int) fmecaEntry.getProbabilityEnum();
						boolean isCritical = fdirParameters.get(0).isCritical(DETECTABLE, severity, probability);
						if (isCritical) {
