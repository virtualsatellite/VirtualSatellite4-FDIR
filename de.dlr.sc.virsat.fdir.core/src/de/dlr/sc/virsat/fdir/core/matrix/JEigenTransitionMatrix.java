 ...
 */
public class JEigenTransitionMatrix extends DenseMatrix implements IMatrix {
	
	/**
	 * @param countStates markov chain state count
	 */
	public JEigenTransitionMatrix(int countStates) {
	...
	public double multiply(double[] vector, double[] result) {
		...
		for (int i = 0; i < countStates; ++i) {
			...
			double[] row = this.row(i).getValues();
	...
	public MatrixIterator getIterator(double[] probabilityDistribution, double delta, double eps) {
		...
		return new JEigenExponentialMatrixIterator(this, probabilityDistribution, delta, eps);
	}
}
