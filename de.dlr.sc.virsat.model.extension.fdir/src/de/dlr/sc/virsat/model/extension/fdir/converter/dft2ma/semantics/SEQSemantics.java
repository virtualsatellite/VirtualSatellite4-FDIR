...
public class SEQSemantics implements INodeSemantics {

	@Override
	public boolean handleUpdate(FaultTreeNode node, DFTState state, DFTState pred, GenerationResult generationResult) {
		...
		for (FaultTreeNode child : children) {
			if (state.isNodeActive(child) && !state.hasFaultTreeNodeFailed(child)) {
				break;
			}
			
			if (!state.isNodeActive(child)) {
				state.setNodeActivation(child, true);
				break;
			}
		}
