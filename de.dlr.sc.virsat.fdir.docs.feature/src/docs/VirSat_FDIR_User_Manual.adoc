...
-- Graphical representation:::
image:faultTrees/{type}_Diagram.png[Representation in graphical editor]:::
This picture shows a free floating comment, not associated with any diagram element. It can be placed or dragged freely on 
...
reassigned to another fault tree element by dragging and dropping it from its currently attached
fault tree element to the new one.
